...
{- 
Questao 2
Crie uma função que converta um quantidade de segundos em uma String no formato "HH:MM:SS". 
O valor máximo possível a ser recebido por essa função é 359999 ("99:59:59").
Ex: converter 86399 = "23:59:59"
-}

converter :: Int -> String
converter tempo = show horas ++ ":" ++ show minutos ++ ":" ++ show segundos
                    where
                        horas = div tempo 3600
                        restoHoras = mod tempo 3600
...
removeMin' [] = []
removeMin' xs = delete (encontrarMenor xs) xs
                    where
                        encontrarMenor (x:xs) = foldr min x xs
