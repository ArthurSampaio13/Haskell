...
-- Questao 2
data ArvoreBinaria a = Folha a | No (ArvoreBinaria a) a (ArvoreBinaria a) deriving Show

existe :: Ord a => a -> ArvoreBinaria a -> Bool 
existe x (Folha y) = x == y
existe x (No esq meio dir) = x == meio || existe esq || existe dir
...
-- Questao 4
divideLista :: [a] -> ([a], [a])
divideLista lista = splitAt (lenght lista `div` 2) lista

balancear :: [a] -> Arvore a
balancear [a] = Folha a
balancear lista = No (balancear esq) (balancear dir)
                    where
                        (esq, dir) = splitAt lista
